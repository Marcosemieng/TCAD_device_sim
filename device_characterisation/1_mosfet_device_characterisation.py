...
def open_csv(input=None):
    ...
    print("Please select 1 .csv files of Id@Vg and/or Id@Vd data about one single device")
    file_paths = askopenfilenames(
        title="Select 1 file containing Id@Vg and/or Id@Vd data about one single device",
...
# Function to add Lch (nm) for the devie being characterised
def add_lch_to_csv(file_path = None):
    """
    Prompts the user for a numeric value via a pop-up window, 
    and writes this value into a new column 'Lch' in a selected .csv file.
    """
    try:
        # Step 1: Check if file path has been given
        if not file_path:
            print("Error: No file selected.")
            return

        # Step 2: Prompt the user for a numeric value
        lch_value = simpledialog.askfloat("Input", "Enter the numeric value for Lch (nm):")
        if lch_value is None:
            print("Error: No value entered.")
            return

        # Convert the value to an integer (remove decimals)
        lch_value = int(lch_value)

        # Step 3: Read the .csv file and add the 'Lch' column
        rows = []
        with open(file_path, mode='r', encoding='utf-8') as csv_file:
            reader = csv.DictReader(csv_file)
            headers = reader.fieldnames

            # Add 'Lch' to the headers if it doesn't already exist
            if 'Lch' not in headers:
                headers.append('Lch')

            for row in reader:
                row['Lch'] = lch_value  # Add the Lch value to each row
                rows.append(row)
        ...
        print(f"Updated .csv file saved to '{file_path}' with Lch = {lch_value}.")
...
# Function to extract Imax (Vgs=Vdd=1.5V) from Id@Vg at Vds=1V
def imax(file_path):
    Vds_bias = 1  # Vds=1V bias value to filter the data: data must be extracted at this Vds=Vdd=1V
    voltages = []
    currents = []
...
# Function to extract Imin (Vg=-3V) from Id@Vg at Vds=Vdd=1V
# If simulation artifacts produce negative currents, it returns the minimum positive current
def imin(file_path):

    Vds_bias = 1  # Data must be extracted at this Vds=Vdd=1V;
    Vgs_min = -3
    voltages = []
    currents = []
    ...
    # Use NumPy's interpolation to extrapolate current at Vgs = 0 and Vgs = 1
    current_at_min_Vg = np.interp(Vgs_min , voltages, currents)

    # Check if the interpolated current is negative (this is necessary to avoid simulations artifacts)
    if current_at_min_Vg < 0:

        # Find the index of the minimum positive current
        positive_currents = currents[currents > 0]  # Filter only positive currents
        if len(positive_currents) > 0:
            min_positive_current = np.min(positive_currents)  # Find the minimum positive current
            index_min_positive = np.where(currents == min_positive_current)[0][0]  # Get its index
            voltage_at_min_positive = voltages[index_min_positive]  # Get the corresponding voltage
            current_at_min_Vg = np.interp(voltage_at_min_positive , voltages, currents)
            # return voltage_at_min_positive  # Return the voltage where the current is minimum and positive
            # return voltage_at_min_positive  # Return the voltage where the current is minimum and positive
        else:
            print("Error: No positive currents found.")
            return None
...
def Vth_CC(file_path, Vds_bias=1):
    ...
    target_current = 1e-5  # TO-DO: convert in A/cm value --> =  1.8e-5 A/cm - (Vgs=Vth@Ids = 1.810-9 (A/um)) 
...
# This function uses the Vth value calculated by Vth_CC
...
def SS(file_path):
    ...
    correction_index = 0 # This must be calculated rigorously; previously = 0.24
...
def rds_arbitrary(file_path,Vds=0.1,Vgs=0.1):
    rds = None
...
def r_tot(file_path,Vds=0.1,Vgs=1):
    ...
    d = 20e-7 # cm (back gate oxide thickness)
...
def compute(a=None):
    ...
    # Step 0.1: Select .csv file
    input_file_path = open_csv()

    # Ensure a file was selected
    if not input_file_path or len(input_file_path) == 0:
        print("Error: No file selected.")
        return
    ...
    input_file_path = input_file_path[0]
     ...
     # Step 0.2: Save the output file in the same directory as the input .csv file
    input_dir = os.path.dirname(input_file_path)  # Get the directory of the input file
    output_file_path = os.path.join(input_dir, "mosfet_parameters.csv")  # Create the output file path
    ...
    try:
        with open(output_file_path, "a", encoding="utf-8") as ofh:
            ...
            ofh.write('%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' % (
                ion_value, ioff_value, ion_off_ratio_value, imax_value, imin_value, imm_ratio_value, vth_ELR_value,
                vth_CC_value, ss_value, dibl_value, n2D, Rtot, None, None, None))
            
        # Add Lch to the CSV file
        add_lch_to_csv(output_file_path)

        print(f"Parameters saved to {output_file_path}")
    except OSError as e:
        print(f"Error: Unable to write to file '{output_file_path}'. {e}")
