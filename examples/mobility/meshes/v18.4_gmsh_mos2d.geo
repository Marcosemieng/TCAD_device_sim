//// Mesh MOSFET plans SD implant - flipped: v18.4
//// 1) oxide thickness = 5 nm; 2) diffusion thickness = 10 nm
//// 3) device width = 600 nm; 4) channel length = 90 nm
...
cd = gate_width/10; // to approximate the minimum dimension of the junction width theoretically
...
y_bulk_top =       0.0;
y_oxide_top =      y_bulk_top + oxide_thickness; // Changed sign
y_oxide_mid =      0.5 * (y_oxide_top + y_bulk_top);
y_gate_top =       y_oxide_top + gate_thickness; // Changed sign
y_gate_mid =       0.5 * (y_gate_top + y_oxide_top);
y_device_top =     y_gate_top + air_thickness; // Changed sign

y_bulk_bottom =    y_bulk_top - device_thickness; // Changed sign
y_bulk_mid =       0.5 * (y_bulk_top + y_bulk_bottom);
y_device_bottom =  y_bulk_bottom - air_thickness; // Changed sign
y_diffusion =      y_bulk_top - diffusion_thickness; // Changed sign
...
// Field[16] = Threshold;
// Field[16].InField = 5;
// Field[16].SizeMin = cd/5;
// Field[16].SizeMax = 10 * cd;
// Field[16].DistMin = cd/2;
// Field[16].DistMax = 10 * cd;
...
// Field[2].LcMin = cd/5;
// Field[2].LcMax = 10 * cd;
// Field[2].DistMin = cd/2;
// Field[2].DistMax = 10 * cd;


////  Channel-to-Oxide interface (OFF)
// Field[14] = Attractor;
...
// Field[15].LcMin = cd/3;
// Field[15].LcMax = cd;
// Field[15].DistMin = 1 * cd;
// Field[15].DistMax = 2 * cd;


////  Box field - Butk contact influence (ON)
// Field[6] = Box;
// Field[6].VIn = 2 * cd;
// Field[6].VOut = 10 * cd;
...
// Field[6].YMin = y_bulk_bottom - device_thickness;


////  Box field - within bulk influence (ON)
// Field[9] = Box;
// Field[9].VIn = cd/10;
// Field[9].VOut = 10 * cd;
...
// Field[7] = Min;
// Field[7].FieldsList = {2,6,9,16};
// Background Field = 7;


////////////////////////////////// START - code from v8 - not flipped //////////////////////////////////
